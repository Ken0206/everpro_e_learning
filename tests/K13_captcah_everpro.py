        ...
        try:  # 等待搜尋的元素出現才繼續下一步
            ...
            try:  # 等待搜尋的元素出現才繼續下一步
                ...
                if next_btn.is_displayed():  # 找到 "next_btn"
                    ...
                    ans_btn = driver.find_element(By.XPATH, '/html/body/div[2]/div/div[1]/div[3]/div[1]/button[1]/div')
                    if ans_btn.is_displayed():  # 找到 "next_btn"
                        ans_btn.click()
...
def find_ans_button():
    try:  # 等待搜尋的元素出現才繼續下一步
        ...
        if ans_button.is_displayed():  # 找到 "ans_button"
            ans = True
            print("找到 ans_button")
    except NoSuchElementException:
        ans = False
...
def ans_questions():
    ...
    while True:
        ...
        for question in questions:
            ...
            ans_text = driver.find_element(By.XPATH, '//*[@id="q_8moe1sobs6p2-x40peundytvl"]/div[2]/div[1]/div[2]/div/div[1]').text
            print(ans_text)
            if ans_text == "錯誤":
                print("回答錯誤,再次作答")
                # 按 "再次作答"
...
def authentication():
    try:
        ...
        driver.find_element(By.ID, 'insuranceNo').send_keys(insuranceNo1)
        driver.find_element(By.XPATH, '/html/body/div/div[1]/div/div/form/button').click()
        driver.close()
        driver.switch_to.window(e_learning_window)  # 切回 e_learning_window 分頁
        ans = True
    except NoSuchElementException:
        print('找不到 身份驗證 進入的 iframe')
        ans = False
    return ans
